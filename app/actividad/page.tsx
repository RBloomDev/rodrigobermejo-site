          ...
          <ul className="mt-4 space-y-6">
            {periodos.map(periodo => {
              ...
              return <li key={periodo} data-periodo={periodo} className="border-t border-border-subtle pt-4">
                <p>Periodo: {periodo}. {registro ? <>Tiempo registrado en editor: {registro.seconds} segundos de actividad registrada. Cobertura temporal: {registro.coverage.dias_con_dato} de {registro.coverage.dias_del_periodo} días del periodo. Cobertura del trabajo: {registro.coverage.del_trabajo}.</> : <>Sin dato publicado. {ausencia ? motivos[ausencia.motivo_de_ausencia] : "El artefacto no declara dato ni motivo para este periodo; no se infiere la causa."}</>}</p>
