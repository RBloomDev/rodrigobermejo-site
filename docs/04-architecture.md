...
4. Las rutas de evidencia están **aisladas del funnel comercial**. Ninguna entrada del funnel importa nada del feed, ni directamente ni por una cadena de imports. La ruta de conversión no puede romperse por un problema de evidencia.
   ...
   - **`app/sobre-mi/page.tsx` no es entrada**, y es una decisión, no un olvido: no monta ningún componente del funnel. Que tampoco lea el feed lo fija `tests/portada-identidad.test.ts` sobre el cierre de imports de la ruta.
...
| §4 · 4 — funnel desacoplado del feed | `scripts/check-funnel-isolation.mjs`: cierre **transitivo** de imports desde las 11 entradas del funnel hacia rutas de evidencia, más la comprobación de que cada entrada existe —una lista con rutas muertas recorrería cero archivos y saldría `OK`—. `tests/portada-identidad.test.ts` le inyecta la violación a propósito y comprueba que sale rojo | Imports dinámicos (`import()` con expresión no literal), re-exports vía alias no resolubles estáticamente, y acoplamiento por copia de código en lugar de import |
