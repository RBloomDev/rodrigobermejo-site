...
- El **rendering**: `/proyectos`, `/proyectos/[slug]`, `/evidencia` — y `/actividad`, junto con `activity.json`, autorizados en V1 por `decisions/0015` §4-A pero **todavía no implementados**
...
4. Las rutas de evidencia están **aisladas del funnel comercial**. Ninguna entrada del funnel importa nada del feed, ni directamente ni por una cadena de imports. La ruta de conversión no puede romperse por un problema de evidencia.

   Las entradas son **once**, y son exactamente las que enumera `FUNNEL_ENTRYPOINTS` en `scripts/check-funnel-isolation.mjs`: los ocho componentes (`Hero`, `Offers`, `FinalCTA`, `Navbar`, `Footer`, `FAQ`, `Problems`, `HowItWorks`) más `app/page.tsx`, `app/layout.tsx` y `app/colaborar/page.tsx`.

   - **`app/page.tsx` y `app/layout.tsx`** entraron en `D-09`: son lo que un visitante carga de verdad, y un import del feed añadido ahí no lo veía nadie.
   - **`app/colaborar/page.tsx`** entró al trasladar el funnel comercial íntegro a esa ruta (`docs/brand/02-arquitectura-y-urls.md` §1). `FUNNEL_ENTRYPOINTS` fija nombres de archivo literales y no se entera solo de que la cadena cambió de casa.
   - **`app/page.tsx` se queda** aunque ya no monte el funnel: la portada de identidad sigue siendo el archivo donde más tienta importar el feed, y sacarla quitaría la defensa justo cuando empieza a hacer falta.
   - **`app/sobre-mi/page.tsx` no es entrada**, y es una decisión, no un olvido: no monta ningún componente del funnel. Que tampoco lea el feed lo fija `tests/portada-sin-feed.test.ts` sobre el cierre de imports de la ruta.

   Esta enumeración iba detrás del guard —decía ocho componentes cuando el script ya vigilaba diez—. Una spec que enumera ocho y un guard que vigila once es una divergencia, y en este repo divergencia es FAIL: si vuelven a separarse, manda esta lista y se corrige el script, o se corrige esta lista, pero no se dejan ambas.
...
Las cinco primeras filas son los invariantes 1–5 de §4, en ese orden. Las seis últimas no son invariantes de §4: dos son la frontera de PII de `03-privacy-and-publication-policy.md` §4, una es el control 5 de la superficie de escritura (§6), y tres son las reglas del canal editorial de `plataforma/02-editorial.md` §8.3 y §5.2. Se tabulan aquí porque tienen el mismo problema de sobredeclaración.
...
| §4 · 2 — feed inválido → build rojo | `npm test` y `npm run build`: `lib/proof/feed.ts` valida con zod y lanza `FeedInvalidoError`; `tests/proof-feed.test.ts` cubre los diez estados de error, cada uno con su caso rojo | Un feed sintácticamente válido que mienta sobre el mundo. El cross-check de `meta.counts` y G2 prueba coherencia **entre archivos**, no que el contenido sea verdad |
| §4 · 3 — feed ausente → build verde | `tests/proof-feed.test.ts` y el gate del Sprint D: con `PROOF_FEED_DIR` apuntando a un directorio inexistente, `leerFeed` devuelve `{estado:"ausente"}` y `npm run build` compila las tres rutas en verde | Que la superficie editorial sin afirmaciones sea la **correcta** — que diga lo que debe decir — sigue siendo revisión visual |
| §4 · 4 — funnel desacoplado del feed | `scripts/check-funnel-isolation.mjs`: cierre **transitivo** de imports desde las 11 entradas del funnel hacia rutas de evidencia, más la comprobación de que cada entrada existe —una lista con rutas muertas recorrería cero archivos y saldría `OK`—. `tests/guard-funnel-falsable.test.ts` le inyecta la violación a propósito —un import de `lib/proof/feed` sobre una copia de `Hero.tsx`— y comprueba que sale rojo nombrando el prefijo alcanzado, además de fijar que `app/page.tsx`, `app/layout.tsx` y `app/colaborar/page.tsx` sigan en `FUNNEL_ENTRYPOINTS` | Imports dinámicos (`import()` con expresión no literal), re-exports vía alias no resolubles estáticamente, y acoplamiento por copia de código en lugar de import |
...
| §6 · control 5 — feed inválido bloquea el merge | `tests/feed-schemas.test.ts` valida el feed **publicado** contra los schemas **publicados**, más los cross-checks de `meta.counts` y G2 que ningún schema puede expresar. Corre en `npm test`, que es un check obligatorio del PR | Que los schemas publicados sean fieles a `docs/05`. Esa es otra comprobación (`tests/proof-schema-paridad.test.ts` la cubre del lado de zod, no del lado del JSON Schema) |
...
| `plataforma/02-editorial.md` §8.3 — el estado del canal no está versionado en este repositorio público | `npm run guard:estado-editorial`: `git ls-files` sobre las dos rutas **históricas literales** (`scripts/editorial/estado`, `scripts/editorial/redacciones`). Falla también si no puede consultar el índice, en vez de pasar sin medir. Y, **solo cuando detecta que la migración ya corrió** —`$EDITORIAL_ESTADO_DIR` con `bitacora.jsonl` dentro—, exige además que esas rutas no existan ya **en disco** (§8.6 punto 4). Sus dos casos rojos están en `scripts/editorial/pruebas/guard-estado.test.mjs`: un árbol de git en `tmpdir` con un archivo de estado rastreado, y otro con la migración corrida a medias | Que el estado siga en el **historial ya publicado**: desrastrear impide que se siga publicando, no despublica lo publicado. Tampoco mira rutas nuevas de estado que alguien añada sin meterlas en la lista. Y **no mira la ruta privada resuelta en ejecución**: esa la vigila otro control, no este —`git ls-files` no diría nada de una ruta de fuera—. La parte de disco **no se mide antes de migrar**, y entonces lo dice: informa `NO MEDIDA`, que no es verde |
| `plataforma/02-editorial.md` §8.3 — la ruta privada resuelta no cae dentro de un árbol de trabajo de git | `comun.mjs` la comprueba **al arrancar**, antes de abrir un archivo, y aborta nombrando la variable y el árbol (`DirectorioVersionado`). Camina hacia arriba buscando `.git` —directorio o archivo, así que cubre worktrees y submódulos— en vez de invocar `git rev-parse`, que necesitaría que la ruta ya existiera. Sus casos rojos están en `scripts/editorial/pruebas/directorio-fuera-de-git.test.mjs`, que corre en `npm test`: este repositorio, una ruta suya que no existe, una ruta relativa, y un repositorio ajeno montado en `tmpdir` | Que el directorio elegido sea **duradero**: fuera de git no es lo mismo que respaldado, y esto no mira la durabilidad (§8.5 de esa spec). Tampoco ve un `.git` creado **después** de que el canal arrancara, ni protege a quien escriba en esas rutas sin pasar por `comun.mjs` |
| `plataforma/02-editorial.md` §5.2 y §8.1 — deduplicación y recuperación de pendientes | `npm run guard:canal`, **sin red y sin inferencia** (redactor falso): 2ª corrida sobre las mismas entradas = 0 nuevos; una corrida muerta por `SIGKILL` deja la entrada pendiente y la siguiente la recupera sin duplicar; tres fallos la descartan con motivo escrito. Su caso rojo lo fuerza `--sin-deduplicacion`, y lo comprueba `guard-canal-falsable.test.mjs` | **Nada sobre el canal con inferencia real**: el redactor falso prueba la máquina de estados, no que un modelo redacte algo publicable. Tampoco cubre `detectar` ni `verificar` contra la red —son dobles con datos locales—, ni la durabilidad del estado, que es otro problema (§8.5 de esa spec) |
...
Corolario, y es la otra mitad del mismo argumento: un guard que nunca ha fallado tampoco prueba que esté conectado. Los cuatro se rompieron a propósito el 2026-08-24 y se comprobó que fallan; las salidas están en `audits/2026-08-24-gate-falsability.md`. Los tres del canal editorial nacieron con su caso rojo automatizado —un archivo por criterio: `scripts/editorial/pruebas/guard-estado.test.mjs`, `guard-canal-falsable.test.mjs` y, para la ruta privada, `directorio-fuera-de-git.test.mjs`—, que es un grado más: su falsabilidad no depende de que alguien repita el experimento a mano, se vuelve a comprobar en cada corrida de `npm test`. Eso los hace falsables, no los convierte en garantías: lo que cada uno no cubre sigue siendo exactamente lo que dice esta tabla.
...
5. CI valida el feed contra `schemas/*.json` en cada PR; feed inválido bloquea el merge. **Implementado el 2026-08-28** (Sprint D), y no como un paso propio de `ci.yml` sino dentro de `npm test`, que es check obligatorio: `tests/feed-schemas.test.ts` compila los cinco JSON Schema publicados con ajv 2020-12 y valida contra ellos el artefacto realmente publicado en `public/proof/v1/`, más los cross-checks de `meta.counts` y G2 que ningún schema puede expresar por ser relaciones **entre** archivos. Si el feed no está publicado el test pasa — la ausencia es un estado legítimo (`05` regla 5) — y no es vacuo: `tests/proof-feed.test.ts` cubre los diez casos en que la ausencia parcial o la corrupción ponen el build en rojo.

   > Anotado el 2026-09-10. Entre el 2026-08-26 y el Sprint D este control estuvo correctamente declarado como no implementado. Lo que fallaba desde el 28 de agosto era lo contrario: seguía diciéndose ausente después de existir, que es el mismo defecto de §4.1 con el signo cambiado — una subdeclaración hace creer que falta trabajo hecho, y el trabajo se vuelve a hacer o el invariante se cree desprotegido.
