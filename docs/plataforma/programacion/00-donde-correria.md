...
|---|---|---|---|
| **GitHub Actions, este repo** | 4 workflows activos (`gh workflow list`), ya sobre **Node 24** (`ci.yml:39`), red saliente. **Cero `schedule`** hoy | Cerrar T-E1 **y T-E3**, **un almacén externo con respaldo** para el estado y los borradores, el archivo de workflow y el secret | **No solo la credencial: faltan T-E1, T-E3 y el almacén** |
...
**Recomendación: GitHub Actions en este mismo repositorio, condicionada a T-E1, a T-E3 y al
...
workflow es la razón por la que, **con T-E1 y T-E3 cerrados los dos**, activar el cron no
cambiaría qué se vuelve público: el runner no necesitaría ninguna ruta hacia el árbol público
del repositorio. **Las dos condiciones son necesarias y ninguna basta sola**, porque son dos
caminos distintos hacia el árbol público:
...
**Hoy no está cerrado ninguno de los dos, y hay que decirlo donde se responde la pregunta:**
...
- El workflow de referencia de este directorio **todavía contiene el paso *persistir estado y
  ...
  y empuja a la rama de trabajo. **Eso es el defecto que T-E3 elimina, no el mecanismo de
...
de `02-editorial.md` §8.1 es trabajo pendiente de **T-E1**. **Eliminar del workflow el paso
*persistir estado y corpus* no es de T-E1: es de T-E3**, que es el trabajo sobre este
directorio. Sus valores concretos viven únicamente en el `.env`
...
- Que el estado sobreviva a una corrida en un runner. **El camino de commitear de vuelta a
  ...
  árbol**: `canal-editorial.yml:183` lo contiene hasta que **T-E3** lo borre —ver el bullet
  de medición de arriba—. Así que lo que hay que probar es el almacén externo con respaldo, y
...
primer paso de prueba una vez cerrados **T-E1 y T-E3**; no debe activarse antes. Con T-E3
abierto, esa corrida «de prueba» commitearía y empujaría igual: el paso *persistir estado y
corpus* no distingue una corrida manual de una programada.
...
1. **Cerrar T-E1, con la compuerta de CI que lo sostiene** (`02-editorial.md` §8.6). Hasta
   entonces el canal escribe el estado y los borradores **dentro** de este repositorio
   público — exactamente lo que `02-editorial.md` §8.3 prohíbe. Y sin la compuerta, cerrar
   T-E1 no impide que un `git add` distraído lo reabra en silencio.
2. **Cerrar T-E3: borrar del workflow el paso *persistir estado y corpus*.** Es el segundo
   camino, y es independiente del primero: mientras ese paso exista, activar el cron commitea
   y empuja lo que encuentre —publica estado sin que nadie lo autorice—, aunque T-E1 ya esté
   cerrado.
